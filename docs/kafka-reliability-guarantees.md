...
| Consumer 인스턴스가 처리 도중 죽음 | 메시지 유실 없음 — auto-commit을 꺼두면 Spring Kafka 컨테이너가 리스너 처리를 정상적으로 마친 뒤에만 offset을 커밋하므로, 죽으면 커밋 전 상태라 리밸런싱 후 재전달된다 | [`KafkaConsumerConfig.java`](../src/main/java/com/uply/coupon/messaging/config/KafkaConsumerConfig.java#L38) (`ENABLE_AUTO_COMMIT_CONFIG=false`) |
...
- **auto-commit을 끈 것**은 Kafka의 at-least-once 전달을 올바르게 활용하는 표준적인
  ...
  Kafka 컨테이너가 리스너 처리를 정상적으로 마친 뒤에만 offset을 커밋하도록 맡긴다. 애플리케이션
  코드가 `Acknowledgment.acknowledge()`를 직접 호출하는 것은 아니다 — `AckMode`를 별도로
  설정하지 않았으므로 기본값(`BATCH`)이 적용되며, 이 값도 "처리 완료 후 커밋"이라는 성격은
  동일하다. 결과적으로 최소 한 번은 반드시 처리된다.
