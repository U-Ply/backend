...
4. 멱등성 확인             (coupon_history 조회
                            → 이력이 있으면 coupons 조회 1회 추가
                              (재사용 요청인지 판정: 발급 이력 여부 + campaignId·userId·stockId 일치 여부))
...
**4의 `coupons` 추가 조회는 같은 idempotencyKey로 재요청이 왔을 때만 실행된다.** PK(`coupon_id`)
단건 조회라 정상 발급 경로(최초 요청)의 길이에는 영향을 주지 않으며, 5.2의 락 대기 실측치(50초대)에
비하면 무시할 수준이다. 다만 V1에서는 이 조회도 3~8 직렬 구간 안에서 일어난다.
