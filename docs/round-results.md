...
## 2026-08-21 회차 (commit 8e49e4f, LOCAL-DOCKER-01)

위 8-20 회차의 "검증하지 못한 것" 중 CLOCK-02 가 해소된 뒤의 첫 세트다.
세 회차 모두 `DB_POOL_SIZE=30`, 요청 100건, 재고 30장, campaign 31 / stock 301.
회차 전 구간을 `scripts/test/acceptance.sh` 가 수행하므로 단계 누락이 불가능하다.

| runId | 회차 | 판정 | 규칙 | 비고 |
| --- | --- | --- | --- | --- |
| `SMOKE-V1-20260821-173652` | V1 | 통과 | 검사 13 / N/A 2 | CLOCK-02·REC-01 은 DB 경로라 N/A |
| `SMOKE-V2-20260821-172411` | V2 | 통과 | 검사 15 / N/A 0 | CLOCK-02 실측 판정 |
| `SMOKE-V3-20260821-172608` | V3 | 통과 | 검사 15 / N/A 0 | lag 0, DLT 0 |
...
- **상태 전이 회차** — 발급 → 사용 → 취소, 그리고 만료 전용 캠페인으로 만료 배치까지.
  만료 배치는 아직 회차에서 한 번도 실행된 적이 없다.
- 렌더러 테스트에 "INV 전부 통과 + REC-01 위반 1건" 케이스 추가.
  이번 판정 버그가 통과했던 이유는 이 조합을 아무도 보지 않았기 때문이다.
