...
public class LuaScriptIssueStrategy implements CouponIssueStrategy {
    ...
    public void init() {
        ...
        issueScript.setScriptSource(new ResourceScriptSource(new ClassPathResource("scripts/issue_coupon.lua")));
        issueScript.setResultType(List.class);
    }
	
	/**
	 * Redis 캐시에 CampaignStock 데이터가 이미 적재되어 있는 상태.
	 * stockId, remainingStock 만 사용
	 * 쿠폰 발급 성공 시 stockId 를 IssueResult 에 포함해야 Kafka 메시지로 전달 가능.
	 * 
	 * Redis Key 추가 필요
	 * stock:{campaignId}:{routeId}:{fareClassId} -> stockId
	 */
    @Override
    public IssueResult issue(Long campaignId, Long routeId, Long fareClassId, Long userId, String idempotencyKey) {
        ...
        List<Object> result = redisTemplate.execute(
                issueScript,
                List.of(mapKey, issuedCampaignKey),
