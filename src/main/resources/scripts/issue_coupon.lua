...
if not openAt then
    return {-3, nowMillis} -- SYSTEM_ERROR: 웜업 데이터 누락
end

if nowMillis < tonumber(openAt) then
    return {-4, nowMillis} -- CAMPAIGN_NOT_OPEN
end
...
if nowMillis >= tonumber(expireAt) then
    return {-5, nowMillis} -- CAMPAIGN_EXPIRED
end

-- 3. [중복 검사] 해당 캠페인에서 이미 발급받은 유저인가?
if redis.call('SISMEMBER', KEYS[2], ARGV[1]) == 1 then
    return {-1, nowMillis} -- ALREADY_ISSUED
end
...
if not currentStock or tonumber(currentStock) <= 0 then
    return {-2, nowMillis} -- OUT_OF_STOCK
end
...
-- 6. 성공 시 {1, nowMillis} 반환 (stockId는 이미 Java 단에서 알고 있으므로 반환 불필요)
return {1, nowMillis}
